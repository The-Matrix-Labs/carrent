...
export function makePayment(
  ...
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      let bookingTypeData = preApprove ? 'instant' : bookingType;

      let isStartValue = startTimeData(startTime), isEndValue = startTimeData(endTime);
      let isStartDate = moment(checkIn).format('YYYY-MM-DD'), isEndDate = moment(checkOut).format('YYYY-MM-DD');

      let checkInDate = isStartDate + ' ' + isStartValue, checkOutDate = isEndDate + ' ' + isEndValue;
      ...
      const { data } = await client.mutate({
        ...
        variables: {
          ...
          hostId,
          guestId,
          checkIn: moment(checkInDate),
          checkOut: moment(checkOutDate),
          guests,
          message,
          basePrice,
          delivery,
