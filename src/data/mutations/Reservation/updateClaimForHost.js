...
import { ClaimImages, Reservation, ThreadItems } from "../../models";
...
const updateClaimForHost = {
    ...
    async resolve({ request }, { reservationId, claimAmount, claimImages, claimReason }) {
        try {
            ...
            await Reservation.update({ claimAmount, claimStatus: 'requested', claimReason }, { where: { id: reservationId } });
            ...
            if (threadItems) await ThreadItems.create({ reservationId, type: 'claimRequested', sentBy: request.user.id, startDate: threadItems.startDate, threadId: threadItems.threadId, endDate: threadItems.endDate, startTime: threadItems.startTime, endTime: threadItems.endTime, personCapacity: threadItems.personCapacity });
            return { status: 200 };
        } catch (error) {
