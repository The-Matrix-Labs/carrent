                        ...
                        !loading && getWishListGroup && <Col xs={12} sm={12} md={12} lg={12} className={cx(s.landingContent, s.noPadding)}>
                            <Col xs={12} sm={12} md={12} lg={12} className={cx(s.noPadding, s.space2)}>
                                <Col xs={12} sm={6} md={6} lg={6} className={s.space2}>
                                    <Link to={"/wishlists"} className={cx(s.modalCaptionLink, s.button, s.btnPrimary, s.btnlarge, s.linkText, s.btnResponsive, s.goListBtn, 'modalCaptionLinkRTL')}>
                                        {formatMessage(messages.goToAllLists)}
                                    </Link>
                                </Col>
                                <Col xs={12} sm={6} md={6} lg={6}>
                                    <div>
                                        <Button className={cx(s.button, s.btnPrimary, s.btnlarge, s.pullRight, s.noMargin, s.btnResponsive, s.space2, 'floatLeftRTL')}
                                    ...
                                    <div>
                                        ...
                                        >
                                            <a href="javascript:void(0)"
                                                className={cx(s.modalCaptionLink, s.pullRight, s.noMargin, s.button, s.btnPrimary, s.btnlarge, s.linkText, s.btnResponsive, 'floatLeftRTL', 'modalCaptionLinkRTL')}>
                                ...
                                !loading && getWishListGroup && <Col xs={12} sm={12} md={12} lg={12}>
                                    {
                                        getWishListGroup.wishListCount > 0 && getWishListGroup.wishLists && getWishListGroup.wishLists.length > 0 && <div className={s.landingContentTitle}>
                                            ...
                                            <label className={cx(s.pullRight, 'floatLeftRTL')}>
                                                <small>{getWishListGroup.wishListCount} {getWishListGroup.wishListCount > 1 ? formatMessage(messages.cars) : formatMessage(messages.car)}</small>
                                            </label>
                                        </div>
                                    }
                                    {
                                        getWishListGroup.wishLists && getWishListGroup.wishLists.length > 0 && getWishListGroup.wishListCount > 0 && <Row>
                                            {
                                                getWishListGroup.wishLists.map((item, index) => {
                                                    if (item.listData != null) {
                                                        ...
                                                        return (
                                                            <Col lg={4} md={4} sm={4} xs={12} className={s.paddingBottom}key={index}>
                                                                <ListingItem
                                                                    id={item.listData.id}
                                                                    ...
                                                                    transmission={item.listData.transmission}
                                                                />
                                                            </Col>
                                                        )
                                                    }
