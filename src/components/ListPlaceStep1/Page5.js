    ...
    return (
      <div>
        <div className={s.landingMainContent}>
          ...
          <FormGroup className={cx(s.formGroup, 'fullAddressSection')}>
            <Field
              name="location"
              component={this.renderPlacesSuggest}
              label={formatMessage(messages.yourFullAddress)}
              className={cx(s.formControlInput, s.jumboInput, s.fullAddressInput)}
            />
          </FormGroup>
        </div>
          ...
          <div className={s.nextBackButton}>
            ...
            <FormGroup className={s.formGroup}>
              <Col xs={12} sm={12} md={12} lg={12} className={s.noPadding}>
                <Button className={cx(s.button, s.btnPrimaryBorder, s.btnlarge, s.pullLeft)} onClick={() => previousPage("car")}>
              ...
              <Col xs={12} sm={12} md={12} lg={12} className={s.noPadding}>
                ...
                }
                {
                  !isExistingList && <Loader
                    type={"button"}
                    label={formatMessage(messages.next)}
                    buttonType={formatMessage(messages.next)}
                    show={loading}
