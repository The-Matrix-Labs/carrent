                ...
                data && data.length > 0 && data.map((value, key) => {
                  return (
                    <Tr key={key}>
                      ...
                      <Td data-label={formatMessage(messages.baseCurrencyLabel)} column={formatMessage(messages.baseCurrencyLabel)} data={value.isBaseCurrency === true ? formatMessage(messages.activeLabel) : ""} />
                      <Td data-label={formatMessage(messages.status)} column={formatMessage(messages.status)}>
                        <span>
                          {
                            value.isEnable && <span> {formatMessage(messages.enabledLabel)} </span>
                          }
                          {
                            !value.isEnable && <span> {formatMessage(messages.disabledLabel)} </span>
                          }
                        </span>
                      </Td>
                      <Td data-label={formatMessage(messages.setEnableDisable)} column={formatMessage(messages.setEnableDisable)}>
                        <a href="javascript:void(0)" onClick={() => this.handleUpdateStatus(value.id, value.isEnable)} >
                          {
                            value.isEnable && <span> {formatMessage(messages.disableLabel)} </span>
                          }

                          {
                            !value.isEnable && <span> {formatMessage(messages.enableLabel)} </span>
                          }
                        </a>
                      </Td>
