    ...
    render() {
        ...
        return (
            <div className={cx(s.pagecontentWrapper, 'pagecontentWrapperRTL')}>
                <div>
                    <h1 className={s.headerTitle}> <FormattedMessage {...messages.paymentGatewaySection} /></h1>
                    <div className={cx('table-responsive', 'tableBorderRadiusAdmin', 'NewAdminResponsiveTable', 'NewResponsiveTableAdmin', 'tableOne')}>
                        <Table className="table" noDataText={formatMessage(messages.noRecordFound)} sortable={true}>
                            ...
                            {
                                getAllPaymentMethods && getAllPaymentMethods.length > 0 && getAllPaymentMethods.map((item, index) => {
                                    return (
                                        <Tr key={index}>
