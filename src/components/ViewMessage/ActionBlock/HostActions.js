...
class HostActions extends Component {
			...
			<Panel className={cx(s.space5, s.contextPadding)}>
				...
				}
				{
					listPublishStatus && <Col md={12} className={cx(s.spaceTop2, s.noPadding)}>
						...
						<Button className={cx(s.btnPrimaryBorder, s.btnRight, 'declineRTL')} onClick={() => this.sendMessage('declined')}>
							<FormattedMessage {...messages.decline} />
						</Button>
					</Col>
				}
	...
	bookingConfirmed() {
		...
		return (
			<Panel className={cx(s.space5, s.contextPadding)}>
				...
				<Col md={12} className={cx(s.spaceTop1, s.noPadding)}>
	...
	completed() {
		return (
			<Panel className={cx(s.space5, s.contextPadding)}>
				<h4><strong><FormattedMessage {...messages.reservationCompleted1} /></strong></h4>
				<p className={s.spaceTop2}>
