        ...
        return (<Modal show={show} onHide={closeModal} className={'claimModalCss'}>
            ...
            <Modal.Body>
                <form onSubmit={handleSubmit(this.handleSubmit)}>
                    <FormGroup className={cx(s.formGroup, s.space4, s.paddingLeftRight)}>
                    ...
                    {claimedByHost && <FormGroup className={cx(s.formGroup, s.space4, s.paddingLeftRight)}>
                        <ControlLabel className={s.landingStep3}>
                    ...
                    <div className={s.refundPadding}>
                        ...
                        <div className={cx(s.alignRight, 'textAlignLeftRTL')}>
                            <Button onClick={closeModal} className={s.cancelBtn}> {formatMessage(messages.cancel)} </Button>
                            <div className={s.disPlayInline}> <Loader type={"button"} buttonType={"submit"} label={formatMessage(messages.submitRefund)} show={loader} disabled={loader} className={s.submitBtn} /> </div>
                        </div>
                    </div>
                </form>
            </Modal.Body>
...
const mapProps = {};
export default injectIntl(graphql(updateClaimRefundMutation, { name: 'updateClaimRefund' })(withStyles(s)(connect(mapState, mapProps)(AdminClaimModal))));