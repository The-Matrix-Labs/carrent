...
class ListItem extends React.Component {
  ...
  render() {
    const { itemList, label, showLabel, hideLabel } = this.props;
                  ...
                  firstArray.map((item, key) => {
                    if (item.listsettings.isEnable === "1") {
                      return (
                        <p
                          key={key}
                          className={cx(s.splitList, s.vtrTop, 'splitListRTL')}
                        >
                          <span>  <FontAwesome.FaAngleRight className={cx(s.arrowIcon, 'arrowIconListRTL')} /> </span>
